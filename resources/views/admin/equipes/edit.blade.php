...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.equipe.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.equipes.update", [$equipe->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label class="required" for="nom">{{ trans('cruds.equipe.fields.nom') }}</label>
                <input class="form-control {{ $errors->has('nom') ? 'is-invalid' : '' }}" type="text" name="nom" id="nom" value="{{ old('nom', $equipe->nom) }}" required>
                @if($errors->has('nom'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('nom_coach') ? 'is-invalid' : '' }}" type="text" name="nom_coach" id="nom_coach" value="{{ old('nom_coach', $equipe->nom_coach) }}" required>
