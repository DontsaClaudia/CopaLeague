            ...
            <form method="POST" action="{{ route('admin.joueurs.store') }}" enctype="multipart/form-data">
                ...
                <div class="form-group">
                    ...
                    <select class="form-control select2 {{ $errors->has('equipe') ? 'is-invalid' : '' }}" name="equipe_id"
                        ...
                        @foreach ($equipes as $id => $entry)
                            <option value="{{ $id }}" {{ old('equipe_id') == $id ? 'selected' : '' }}>
                                {{ $entry }}</option>
                        @endforeach
                    </select>
                ...
                <div class="form-group">
                    ...
                    <input class="form-control {{ $errors->has('nom_et_prenom') ? 'is-invalid' : '' }}" type="text"
                        name="nom_et_prenom" id="nom_et_prenom" value="{{ old('nom_et_prenom', '') }}" required>
                ...
                <div class="form-group">
                    ...
                    <input class="form-control {{ $errors->has('age') ? 'is-invalid' : '' }}" type="number" name="age"
                        id="age" value="{{ old('age', '') }}" step="1" required>
                    @if ($errors->has('age'))
                ...
                <div class="form-group">
                    ...
                    <input class="form-control {{ $errors->has('dossard') ? 'is-invalid' : '' }}" type="number"
                        name="dossard" id="dossard" value="{{ old('dossard', '') }}" step="1" required>
                ...
                <div class="form-group">
                    ...
                    <input class="form-control {{ $errors->has('poste') ? 'is-invalid' : '' }}" type="text"
                        name="poste" id="poste" value="{{ old('poste', '') }}" required>
                    @if ($errors->has('poste'))
