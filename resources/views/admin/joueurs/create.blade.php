    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.joueurs.store") }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('equipe') ? 'is-invalid' : '' }}" name="equipe_id" id="equipe_id" required>
                    @foreach($equipes as $id => $entry)
                        <option value="{{ $id }}" {{ old('equipe_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('nom_et_prenom') ? 'is-invalid' : '' }}" type="text" name="nom_et_prenom" id="nom_et_prenom" value="{{ old('nom_et_prenom', '') }}" required>
