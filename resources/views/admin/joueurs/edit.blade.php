        ...
        <div class="card-body">
            <form method="POST" action="{{ route('admin.joueurs.update', [$joueur->id]) }}" enctype="multipart/form-data">
                ...
                <div class="form-group">
                    ...
                    <select class="form-control select2 {{ $errors->has('equipe') ? 'is-invalid' : '' }}" name="equipe_id"
                        id="equipe_id" required>
                        @foreach ($equipes as $id => $entry)
                            <option value="{{ $id }}"
                                {{ (old('equipe_id') ? old('equipe_id') : $joueur->equipe->id ?? '') == $id ? 'selected' : '' }}>
                                {{ $entry }}</option>
                        @endforeach
                    </select>
                ...
                <div class="form-group">
                    ...
                    <input class="form-control {{ $errors->has('nom_et_prenom') ? 'is-invalid' : '' }}" type="text"
                        name="nom_et_prenom" id="nom_et_prenom" value="{{ old('nom_et_prenom', $joueur->nom_et_prenom) }}"
                        required>
                ...
                <div class="form-group">
                    <label class="required"
                        for="date_de_naissance">{{ trans('cruds.joueur.fields.date_de_naissance') }}</label>
                    <input class="form-control date {{ $errors->has('date_de_naissance') ? 'is-invalid' : '' }}"
                        type="text" name="date_de_naissance" id="date_de_naissance"
                        value="{{ old('date_de_naissance', $joueur->date_de_naissance) }}" required>
                ...
                <div class="form-group">
                    ...
                    <input class="form-control {{ $errors->has('age') ? 'is-invalid' : '' }}" type="number" name="age"
                        id="age" value="{{ old('age', $joueur->age) }}" step="1" required>
                    @if ($errors->has('age'))
                ...
                <div class="form-group">
                    ...
                    <input class="form-control {{ $errors->has('dossard') ? 'is-invalid' : '' }}" type="number"
                        name="dossard" id="dossard" value="{{ old('dossard', $joueur->dossard) }}" step="1"
                        required>
                ...
                <div class="form-group">
                    ...
                    <input class="form-control {{ $errors->has('poste') ? 'is-invalid' : '' }}" type="text"
                        name="poste" id="poste" value="{{ old('poste', $joueur->poste) }}" required>
                    @if ($errors->has('poste'))
