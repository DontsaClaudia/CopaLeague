    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.matches.update", [$match->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('resultat_1') ? 'is-invalid' : '' }}" type="number" name="resultat_1" id="resultat_1" value="{{ old('resultat_1', $match->resultat_1) }}" step="1">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('resultat_2') ? 'is-invalid' : '' }}" type="number" name="resultat_2" id="resultat_2" value="{{ old('resultat_2', $match->resultat_2) }}" step="1">
            ...
            <div class="form-group">
                ...
                <input class="form-control datetime {{ $errors->has('date_et_heure') ? 'is-invalid' : '' }}" type="text" name="date_et_heure" id="date_et_heure" value="{{ old('date_et_heure', $match->date_et_heure) }}">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('stade') ? 'is-invalid' : '' }}" name="stade_id" id="stade_id" required>
                    @foreach($stades as $id => $entry)
                        <option value="{{ $id }}" {{ (old('stade_id') ? old('stade_id') : $match->stade->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('stade'))
